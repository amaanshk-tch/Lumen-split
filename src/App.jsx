...
const CONTRACT_ID = "CBRY2H3DOGBBO3NKECWQWXKXS3A2DLOIKJMA2DPVXAE5JF62S24IWDHV";
...
export default function App() {
  ...
  const checkRegistration = useCallback(async () => {
    ...
    try {
      const reg = await callRead("is_registered", [
        new Address(publicKey).toScVal(),
      ]);
      if (reg === true) {
        setIsRegistered(true);
        const name = await callRead("get_user_name", [
          new Address(publicKey).toScVal(),
        ]);
        if (name) {
  ...
  const loadGroup = useCallback(
    async (groupId) => {
      ...
      setPayer(
        normalized.members.find((m) => m.address === publicKey)?.address || "",
      );
      setSettleTo("");
      const checks = {};
  ...
  const refreshGroups = useCallback(async () => {
    if (!publicKey) return;
    const ids =
      (await callRead("get_groups_for_member", [
        new Address(publicKey).toScVal(),
      ])) || [];
    const parsed = ids
    ...
    const groupsRaw = await Promise.all(
      parsed.map((gid) => callRead("get_group_with_balances", [toU32(gid)])),
    );
  ...
  useEffect(() => {
    ...
    checkRegistration();
    refreshGroups();
  }, [connected, publicKey, checkRegistration, refreshGroups]);
  ...
  const signAndSubmit = async (buildFn) => {
    ...
    try {
      ...
      if (
        ...
        updateStatus("Account not funded. Please fund it first.", "error");
    ...
    if (selectedWallet === WALLET_TYPES.FREIGHTER) {
      const signedRaw = await signTransaction(prepared.toXDR(), {
    ...
    if (result.status === "ERROR") {
      throw new Error(
        `Submission failed: ${JSON.stringify(result.errorResultXdr || result)}`,
      );
    }
  ...
  const runWrite = async (method, args, okMsg) => {
    ...
    try {
      const contract = new Contract(CONTRACT_ID);
      const hash = await signAndSubmit((account) =>
  ...
  const handleFreighterClick = () => {
    connectWithWallet(WALLET_TYPES.FREIGHTER);
  };
  ...
  return (
    <div className="app">
      ...
      <div className={"card " + (connected ? "compact" : "boxed")}>
        ...
        {!connected ? (
          <>
            <button className="button connect-main-btn" onClick={connectWallet}>
              Connect Wallet
            </button>
          ...
          <>
            <div className="wallet-header-row">
              <div className="wallet-info-block">
                ...
                {isCheckingReg ? (
                  <div className="warning-pill" style={{ opacity: 0.7 }}>Checking profile...</div>
                ) : isRegistered ? (
              ...
              <div className="transition-fade">
                {activeTab === "profile" && (
                  <div className="form-group">
                    <h3>User Profile</h3>
                    {isCheckingReg ? (
                      <p className="tab-hint" style={{ opacity: 0.7 }}>Loading profile data...</p>
                    ) : isRegistered ? (
                      ...
                      <>
                        <p className="tab-hint">
                          Register your name on the Stellar ledger to get
                          started.
                        </p>
                        <input
                          className="input"
                ...
                {activeTab === "add-group" && (
                  <div className="form-group">
                    ...
                    <input
                      className="input"
                      placeholder="Members (comma separated G...)"
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try { members.forEach(m => new Address(m)); } catch { return updateStatus("Invalid address", "error"); }
                    ...
                    {selectedGroup && (
                      <>
                        ...
                        <button className="button" onClick={async () => {
                          ...
                          try { new Address(addMemberAddress); } catch { return updateStatus("Invalid address", "error"); }
                ...
                {activeTab === "add-expense" && (
                  <div className="form-group">
                    <h3>Add Expense</h3>
                    <p className="tab-hint">
                      Split expenses with your group members.
                    </p>
                    <select
                      className="input select-custom"
                      value={selectedGroupId}
                    ...
                    {selectedGroup && (
                      <>
                        ...
                        >
                          Per person share:{" "}
                          <strong>{share.toFixed(2)} XLM</strong>
                        </div>
                        <button className="button" onClick={async () => {
                          if (!payer) return updateStatus("Payer required", "error");
                          if (!billAmount || Number(billAmount) <= 0) return updateStatus("Invalid amount", "error");
                          if (selectedParticipants.length === 0) return updateStatus("No participants", "error");
                    ...
                    {selectedGroup && (
                      <>
                        ...
                        />
                          ...
                          if (!settleAmount || Number(settleAmount) <= 0) return updateStatus("Invalid amount", "error");
                          ...
                          >
                            ...
                            {settlements.map((s, i) => (
                              ...
                              >
                                ...
                                pays{" "}
                                <strong>{formatAmount(s.amount)} XLM</strong> to{" "}
                                {selectedGroup.members.find(
                                  (m) => m.address === s.to,
                                )?.name || short(s.to)}
                              </div>
                  ...
                  <div className="form-group">
                    {recentTransactions.length > 0 && (
                      ...
                      >
                        ...
                        <p className="tab-hint">
                          Your recent transactions from this session.
                        </p>
                    ...
                    {selectedGroup && (
                      ...
                      >
                        {activities.length === 0 && (
                          <p className="tab-hint">No activity yet.</p>
                        )}
                        {activities.map((a) => (
                          <div
                            key={a.id}
                          ...
                          >
                            ...
                            <div style={{ opacity: 0.9 }}>
                              ...
                              {a.kind === "Settlement" &&
                                ` paid ${formatAmount(a.amount)} XLM to ${selectedGroup.members.find((m) => m.address === a.recipient)?.name || short(a.recipient)}`}
                              {a.kind === "MemberAdded" &&
                                (a.amount === 0
                                  ? ` created the group`
                                  : ` added ${selectedGroup.members.find((m) => m.address === a.recipient)?.name || short(a.recipient)}`)}
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            {lastTxHash && activeTab === lastTxTab && (
              ...
              >
                ...
                >
                  ...
                  >
                    Hash:{" "}
                    <span style={{ userSelect: "all" }}>{lastTxHash}</span>
                  </span>
                </div>
                <button
                  style={{
